...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Nova_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local NovaFolder = "Nova"
local ConfigurationFolder = NovaFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		NovaOpen = {Type = 'bind', Value = 'K', Name = 'Nova Keybind'},
		-- buildwarnings
		-- Novaprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(NovaFolder) then
				if isfile and isfile(NovaFolder..'/settings'..ConfigurationExtension) then
					file = readfile(NovaFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"NovaOpen":{"Value":"K","Type":"bind","Name":"Nova Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Nova Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Nova had an issue accessing configuration saving capability.')
		end
	end
end
...
--		warn('Nova Interface Suite | Sirius Analytics:\n\n\nAs you don\'t have file functionality with your executor, we are unable to save whether you want to opt in or out to analytics.\nIf you do not want to take part in anonymised usage statistics, let us know in our Discord at https://discord.gg/ZP4Uy4eyyn and we will manually opt you out.')
--		analytics = true	
--	else
--		prompt.create(
--			'Help us improve',
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Nova", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Nova developers at https://discord.gg/ZP4Uy4eyyn.")
				end
			end)
...
local Nova = useStudio and script.Parent:FindFirstChild('Nova') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Nova:FindFirstChild('Build') and Nova.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Nova | Build Mismatch')
		print('Nova may encounter issues as you are running an incompatible interface version ('.. ((Nova:FindFirstChild('Build') and Nova.Build.Value) or 'No Build') ..').\n\nThis version of Nova is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Nova = Nova, useStudio and script.Parent:FindFirstChild('Nova') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Nova)
	Nova.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Nova.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Nova.Name and Interface ~= Nova then
			Interface.Enabled = false
			Interface.Name = "Nova-Old"
		end
	end
elseif not useStudio then
...
if Nova.AbsoluteSize.X < minSize.X and Nova.AbsoluteSize.Y < minSize.Y then
...
local Main = Nova.Main
local MPrompt = Nova:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Nova:FindFirstChild('Drag')
...
Nova.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Nova/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = Nova.Notifications

local SelectedTheme = NovaLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = NovaLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Nova.Main.BackgroundColor3 = SelectedTheme.Background
	Nova.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Nova.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Nova.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Nova.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Nova.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Nova.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Nova.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Nova.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Nova:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Nova | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Nova | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Nova had an issue decoding the configuration file, please try delete the file and reopen Nova.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(NovaLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Nova | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--NovaLibrary:Notify({Title = "Nova Flags", Content = "Nova was unable to find '"..FlagName.. "' in the save file. Check https://discord.gg/ZP4Uy4eyyn for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(NovaLibrary.Flags) do
...
function NovaLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Nova | Not seeing your actions in notifications?')
			print("Notification Actions are being sunset for now, keep up to date on when they're back in the discord. (https://discord.gg/ZP4Uy4eyyn)")
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			NovaLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Nova'.", Duration = 7, Image = 4400697855})
		else
			NovaLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.NovaOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(NovaFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Nova Settings', 0, true)

	if TabList['Nova Settings'] then
		TabList['Nova Settings'].LayoutOrder = 1000
	end

	if Elements['Nova Settings'] then
		Elements['Nova Settings'].LayoutOrder = 1000
	end
...
function NovaLibrary:CreateWindow(Settings)
	if Nova:FindFirstChild('Loading') then
		if getgenv and not getgenv().NovaCached then
			Nova.Enabled = true
			Nova.Loading.Visible = true

			task.wait(1.4)
			Nova.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				NovaLibrary:Notify({Title = 'Build Mismatch', Content = 'Nova may encounter issues as you are running an incompatible interface version ('.. ((Nova:FindFirstChild('Build') and Nova.Build.Value) or 'No Build') ..').\n\nThis version of Nova is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(NovaFolder) then
		makefolder(NovaFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Nova"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Nova Interface Suite" then
		LoadingFrame.Version.Text = "Nova UI"
	end
	...
	if not Settings.DisableNovaPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				NovaLibrary:Notify({
					Title = "Nova Interface",
					Content = "Enjoying this UI library? Find it at https://discord.gg/ZP4Uy4eyyn",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(NovaFolder.."/Discord Invites") then
			makefolder(NovaFolder.."/Discord Invites")
		end

		if isfile and not isfile(NovaFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(NovaFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Nova RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(NovaFolder.."/Key System") then
			makefolder(NovaFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Nova | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Nova specific development.')
				end
			end
		end
		...
		if isfile and isfile(NovaFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(NovaFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(NovaFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						NovaLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nova | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Nova specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= NovaLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					NovaLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Nova.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Nova | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Nova specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					NovaLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Nova | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Nova specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					NovaLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Nova | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Nova specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					NovaLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= NovaLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nova | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Nova specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						NovaLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Nova | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Nova specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					NovaLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			NovaLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			NovaLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Nova had an issue creating settings.') end
...
function NovaLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function NovaLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function NovaLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Nova Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.NovaOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function NovaLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				NovaLibrary:Notify({Title = "Nova Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			NovaLibrary:Notify({Title = "Nova Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Nova Configurations Error | '..tostring(result))
			NovaLibrary:Notify({Title = "Nova Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = NovaLibrary:CreateWindow({
		Name = "Nova Example Window",
		LoadingTitle = "Nova Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Nova may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Nova to get the key from
	...
	--NovaLibrary:Notify({Title = "Nova Interface", Content = "Welcome to Nova. These - are the brand new notification design for Nova, with custom sizing and Nova calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(NovaLibrary.Theme) do
